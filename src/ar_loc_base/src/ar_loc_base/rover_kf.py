...
class RoverKF(RoverKinematics):
	...
	def predict(self, motor_state, drive_cfg, encoder_precision):
		...
		B = mat([[cos(self.X[2,0]), -sin(self.X[2,0]), 0], 
				  [sin(self.X[2,0]),  cos(self.X[2,0]), 0],
				  [         0,           0, 1]]);
		# ultimately : 
		self.X =  A*self.X + B*dX
		self.P = self.P + diag((encoder_precision**2, encoder_precision**2, encoder_precision**2))
	...
	def update_ar(self, Z, L, uncertainty):
		...
		xr, yr, theta = self.X[:,0]
		theta = self.X[2,0]
		
		H = mat([[-cos(theta),  sin(theta), -(L[0]-xr)*sin(theta) - cos(theta)*(L[1]-yr)], \
				 [-sin(theta), -cos(theta),  cos(theta)*(L[0]-xr) - sin(theta)*(L[1]-yr)]]) 
				 
		K = self.P*H.T*inv(H*self.P*H.T+R)
		self.X = self.X + K*(Z - self.getRotation(self.X[2,0])*(L-self.X[:-1]))
		self.P = (mat([[1,0,0], [0,1,0], [0,0,1]])-K*H)*self.P
