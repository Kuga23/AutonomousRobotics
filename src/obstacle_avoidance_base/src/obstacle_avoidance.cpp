...
class ObstacleAvoidance {
    ...
    protected: // ROS Callbacks
        ...
        geometry_msgs::Twist findClosestAcceptableVelocity(const geometry_msgs::Twist & desired) {
			...
			min_v = std::max(-max_linear_velocity_,  current_velocity_.linear.x - time_horizon_*max_linear_accel_);
            max_v = std::min( max_linear_velocity_,  current_velocity_.linear.x + time_horizon_*max_linear_accel_);
            min_w = std::max(-max_angular_velocity_, current_velocity_.angular.z- time_horizon_*max_angular_accel_);
            max_w = std::min( max_angular_velocity_, current_velocity_.angular.z + time_horizon_*max_angular_accel_);
            ...
            for (unsigned int j=0;j<n_v;j++) {
                ...
                for (unsigned int i=0;i<n_w;i++) {
                    ...
                    ROS_INFO("(%d,%d) Va=%d, v=%f, desV=%f, ang=%f, w=%d",i,j,Va(i,j),v, desired.linear.x,desired.angular.z,w);
                    scores(j,i) = Va(j,i)*exp(-(pow(v-desired.linear.x,2)*k_v_+pow(w-desired.angular.z,2)*k_w_)/2);
                    ROS_INFO("exp=%f, pix %d",exp(-(pow(v-desired.linear.x,2)*k_v_+pow(w-desired.angular.z,2)*k_w_)/2), scores(j,i));
                    if(Va(j,i)==OCCUPIED) scores(j,i) =0;
    ...
    public:
        ObstacleAvoidance() : nh_("~") {
            ...
            nh_.param("k_v",k_v_,0.5);
            nh_.param("k_w",k_w_,0.7);
