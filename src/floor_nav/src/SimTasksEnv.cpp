    ...
    paused(false), manualControl(true), joystick_topic("/teleop/twistCommand"), auto_topic("/mux/autoCommand"), base_frame("/bubbleRob"), reference_frame("/world")
{
    ...
    faceSub = nh.subscribe("/FacesDetect",1,&SimTasksEnv::fdCallback,this);
...
void SimTasksEnv::fdCallback(const face_detect_base::Faces msg) {
	
	
    if(msg.ROI.size()>0){		
		unsigned int max_width = 0;
		unsigned int max_i = 0;
		for(unsigned int i =0; i<msg.ROI.size(); i++){
			if (msg.ROI[i].width > max_width){
				max_i = i;
				max_width=msg.ROI[i].width;
			}
				
		}
		if(msg.ROI[max_i].width*msg.ROI[max_i].height > 800){		
			facedetected=1;
			sensor_msgs::RegionOfInterest faceMSG = msg.ROI[max_i];
			unsigned int center = faceMSG.x_offset + faceMSG.width / 2;
			diff= 64 - center;
